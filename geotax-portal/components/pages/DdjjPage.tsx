...
const ICON_FOLDER = String.fromCodePoint(0x1F4C1)
      ...
      <div className="table-card">
        ...
        {clientData.ddjj.length === 0 ? (
          <div className="empty-state">
            <div className="empty-icon">{ICON_DOC}</div>
            No hay DDJJ cargadas a?n.
          </div>
        ) : (
            ...
            {Object.entries(grupos).map(([tipo, registros]) => (
              <div key={tipo} className="folder">
                <div className="folder-header" onClick={() => toggleFolder(tipo)}>
                  <span className="folder-icon">{ICON_FOLDER}</span>
